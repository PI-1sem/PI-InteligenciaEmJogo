...
public class PerguntaAlternativaDAO {
    public static List<PerguntaAlternativa> listarPerguntasAlternativas() throws Exception{
        ...
        var sql="SELECT p.id_pergunta, p.enunciado, p.nivel_dificuldade, m.nome as materia, a.id_alternativa, a.letra, a.texto, pa.correta FROM pergunta_alternativa pa JOIN pergunta p ON pa.id_pergunta= p.id_pergunta JOIN materia m ON p.id_materia = m.id_materia JOIN alternativa a ON pa.id_alternativa= a.id_alternativa ORDER BY p.id_pergunta, a.letra";
    ...
    public static List<PerguntaAlternativa> listarPerguntasAlternativasExpecificas(String nomeMateria) throws Exception{
        var conectionFactory= new ConnectionFactoryTest();
        var sql="SELECT p.id_pergunta, p.enunciado, p.nivel_dificuldade, m.nome as materia, a.id_alternativa, a.letra, a.texto, pa.correta FROM pergunta_alternativa pa JOIN pergunta p ON pa.id_pergunta= p.id_pergunta JOIN materia m ON p.id_materia = m.id_materia JOIN alternativa a ON pa.id_alternativa= a.id_alternativa WHERE m.nome = ? ORDER BY p.id_pergunta, a.letra";
        var todasPerguntasAlternativas= new ArrayList<PerguntaAlternativa>();
        try(
            var conexao= conectionFactory.obterConexao();
            var ps= conexao.prepareStatement(sql);
